...
func TestFullWindowReceive(t *testing.T) {
	...
	// Fill up the window w/ 2*rcvBufSz= 20 bytes as netstack doubles the value
	// provided when setting receive buffer size.
	data := make([]byte, 20)
	for i := range data {
		data[i] = byte(i)
	}
...
func TestNoWindowShrinking(t *testing.T) {
	...
	checker.IPv4(t, c.GetPacket(),
		checker.TCP(
			...
			checker.AckNum(uint32(seqNum)),
			checker.TCPFlags(header.TCPFlagAck),
			checker.Window(uint16(rcvBufSize/2)>>c.RcvdWindowScale),
		),
	)
}
...
func TestZeroScaledWindowReceive(t *testing.T) {
	...
	checker.IPv4(t, c.GetPacket(),
		...
		checker.TCP(
			...
			checker.SeqNum(uint32(c.IRS)+1),
			checker.AckNum(uint32(790+sent)),
			checker.WindowGreaterThanEq(uint16(defaultMTU>>ws)),
			checker.TCPFlags(header.TCPFlagAck),
		),
	)
}
...
func TestReceiveBufferAutoTuningApplicationLimited(t *testing.T) {
	...
	checker.IPv4(t, p, checker.TCP(
		checker.AckNum(uint32(wantAckNum)),
