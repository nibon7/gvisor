...
type NDPConfigurations struct {
	...
	// addresses will be generated for an IPv6 endpoiint in response to receiving
	// a new Prefix Information option with its Autonomous Address
	...
	// addresses will be generated for an IPv6 endpoint as part of SLAAC privacy
	// extensions, RFC 4941.
	//
...
func (ndp *ndpState) startDuplicateAddressDetection(addr tcpip.Address, ref stack.AddressEndpoint) *tcpip.Error {
	...
	timer = ndp.ep.stack.Clock().AfterFunc(0, func() {
		...
		if !dadDone {
			...
			ref := ndp.ep.getRefOrCreateTempLocked(header.IPv6Any, true /* createTemp */, stack.NeverPrimaryEndpoint)
...
func (ndp *ndpState) sendDADPacket(addr tcpip.Address, ref stack.AddressEndpoint) *tcpip.Error {
	...
	r, err := ndp.ep.stack.FindRoute(ndp.ep.NICID(), header.IPv6Any, snmc, ProtocolNumber, false)
	if err != nil {
		return err
	}
	//r := makeRoute(header.IPv6ProtocolNumber, ref.AddressWithPrefix().Address, snmc, ndp.ep.linkEP.LinkAddress(), ref, false, false)
	defer r.Release()
...
func (ndp *ndpState) handleRA(ip tcpip.Address, ra header.NDPRouterAdvert) {
	...
	// stack's forwarding flag to determine if the IPv6 endpoint is forwarding
	// endoint.
	if !ndp.configs.HandleRAs || ndp.ep.stack.Forwarding() {
		return
	}
...
// addSLAACAddr adds a SLAAC address to the IPv6 endpoint.
//
// The IPv6 endpoint that ndp belongs to MUST be locked.
func (ndp *ndpState) addSLAACAddr(addr tcpip.AddressWithPrefix, configType stack.AddressConfigType, deprecated bool) stack.AddressEndpoint {
	...
	ref, err := ndp.ep.addPermanentAddressLocked(addr, stack.FirstPrimaryEndpoint, configType, deprecated)
	if err != nil {
...
func (ndp *ndpState) deprecateSLAACAddress(ref stack.AddressEndpoint) {
	...
	if ndpDisp := ndp.ep.protocol.ndpDisp; ndpDisp != nil {
		ndpDisp.OnAutoGenAddressDeprecated(ndp.ep.NICID(), ref.AddressWithPrefix())
	}
}
...
func (ndp *ndpState) invalidateSLAACPrefix(prefix tcpip.Subnet, state slaacPrefixState) {
	if r := state.stableAddr.ref; r != nil {
		...
		if err := ndp.ep.removePermanentEndpointLocked(r, false /* allowSLAACInvalidation */); err != nil {
...
func (ndp *ndpState) invalidateTempSLAACAddr(tempAddrs map[tcpip.Address]tempSLAACAddrState, tempAddr tcpip.Address, tempAddrState tempSLAACAddrState) {
	...
	if err := ndp.ep.removePermanentEndpointLocked(tempAddrState.ref, false /* allowSLAACInvalidation */); err != nil {
...
//
// If keepLinkLocal is false, the SLAAC generated link-local address will
// be removed.
//
// The IPv6 endpoint that ndp belongs to MUST be locked.
...
//
// This function will invalidate all discovered on-link prefixes, discovered
...
func (ndp *ndpState) startSolicitingRouters() {
	...
	ndp.rtrSolicit.timer = ndp.ep.stack.Clock().AfterFunc(delay, func() {
		...
		ref := ndp.ep.primaryEndpointRLocked(header.IPv6AllRoutersMulticastAddress, false)
		if ref == nil {
			ref = ndp.ep.getRefOrCreateTempLocked(header.IPv6Any, true /* createTemp */, stack.NeverPrimaryEndpoint)
		}
		ndp.ep.mu.Unlock()
		...
		r, err := ndp.ep.stack.FindRoute(ndp.ep.NICID(), localAddr, header.IPv6AllRoutersMulticastAddress, ProtocolNumber, false)
		if err != nil {
			return
		}
		// r := makeRoute(header.IPv6ProtocolNumber, localAddr, header.IPv6AllRoutersMulticastAddress, ndp.ep.linkEP.LinkAddress(), ref, false, false)
		defer r.Release()
