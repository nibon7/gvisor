...
func NewAddressableEndpointState(ep NetworkEndpoint) *AddressableEndpointState {
	a := &AddressableEndpointState{ep: ep}
...
func (a *AddressableEndpointState) AllAddresses() []tcpip.AddressWithPrefix {
	a.mu.RLock()
	defer a.mu.RUnlock()
...
func (a *addressState) NetworkEndpoint() NetworkEndpoint {
	return a.addressableEndpoint.ep
}
...
func (a *addressState) IsAssigned(spoofingOrPromiscuous bool) bool {
	if !a.addressableEndpoint.ep.Enabled() {
		return false
	}
