...
//
// Callers may call into this interface at anytime. That is, implementations
// must guarantee that calling into the interface will not block
// forever/deadlock.
...
type NIC struct {
	...
	//
	// May be read using only atomic operations but must be written using atomic
	// operations and while mu is write locked. This is so that we can read the
	// enabled status at any point in time without worrying about currently held
	// locks. This is also safe since all writes to enabled are protected by
	// the NIC's lock.
	enabled uint32
...
func (n *NIC) disableLocked() *tcpip.Error {
	...
	atomic.StoreUint32(&n.enabled, 0)
...
func (n *NIC) enable() *tcpip.Error {
	...
	atomic.StoreUint32(&n.enabled, 1)
...
func (n *NIC) primaryEndpoint(protocol tcpip.NetworkProtocolNumber, remoteAddr tcpip.Address) AddressEndpoint {
	n.mu.RLock()
	defer n.mu.RUnlock()
...
func (n *NIC) getAddress(protocol tcpip.NetworkProtocolNumber, dst tcpip.Address) AddressEndpoint {
	return n.getAddressOrCreateTemp(protocol, dst, CanBePrimaryEndpoint, promiscuous)
}

// findEndpoint finds the endpoint, if any, with the given address.
func (n *NIC) findEndpoint(protocol tcpip.NetworkProtocolNumber, address tcpip.Address, peb PrimaryEndpointBehavior) AddressEndpoint {
	return n.getAddressOrCreateTemp(protocol, address, peb, spoofing)
}

// getAddressEpOrCreateTemp returns the referenced network endpoint for the
// given protocol and address.
//
...
func (n *NIC) getAddressOrCreateTemp(protocol tcpip.NetworkProtocolNumber, address tcpip.Address, peb PrimaryEndpointBehavior, tempRef getAddressBehaviour) AddressEndpoint {
	n.mu.RLock()
...
func (n *NIC) getAddressOrCreateTempInner(protocol tcpip.NetworkProtocolNumber, address tcpip.Address, createTemp bool, peb PrimaryEndpointBehavior) AddressEndpoint {
...
func (n *NIC) handlePacket(protocol tcpip.NetworkProtocolNumber, dst, src tcpip.Address, remotelinkAddr tcpip.LinkAddress, ref AddressEndpoint, pkt *PacketBuffer) {
	r := makeRoute(protocol, dst, src, n, ref, false /* handleLocal */, false /* multicastLoop */)
	r.RemoteLinkAddress = remotelinkAddr
	ref.NetworkEndpoint().HandlePacket(&r, pkt)
	ref.DecRef()
}
...
func (n *NIC) DeliverNetworkPacket(remote, local tcpip.LinkAddress, protocol tcpip.NetworkProtocolNumber, pkt *PacketBuffer) {
	...
	if ref := n.getAddress(protocol, dst); ref != nil {
		n.handlePacket(protocol, dst, src, remote, ref, pkt)
		return
	}
	...
	if n.stack.Forwarding() {
		...
		// Found a NIC.
		n := r.nic
		ref := n.getAddressOrCreateTempInner(protocol, dst, false, NeverPrimaryEndpoint)
		ok := ref != nil && n.isValidForOutgoing(ref)
		if ok {
...
func (n *NIC) isValidForOutgoing(ep AddressEndpoint) bool {
	n.mu.RLock()
	defer n.mu.RUnlock()
	return n.Enabled() && ep.IsAssigned(n.mu.spoofing)
}
