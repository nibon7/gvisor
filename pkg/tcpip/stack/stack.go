...
type Stack struct {
	...
	// forwarding is set to 1 when the NIC is enabled and 0 when it is disabled.
	//
...
func (s *Stack) SetForwarding(enable bool) {
	...
	if enable {
		atomic.StoreUint32(&s.forwarding, 1)
	} else {
		atomic.StoreUint32(&s.forwarding, 0)
	}
	...
	for _, nic := range s.nics {
		for _, ep := range nic.networkEndpoints {
			fep, ok := ep.(ForwardingNetworkEndpoint)
			if !ok {
				continue
			}

			fep.SetForwarding(enable)
		}
	}
}
...
func (s *Stack) Forwarding() bool {
	// TODO(igudger, bgeffon): Expose via /proc/sys/net/ipv4/ip_forward.
	return atomic.LoadUint32(&s.forwarding) == 1
}
...
func (s *Stack) getAddressEP(nic *NIC, localAddr, remoteAddr tcpip.Address, netProto tcpip.NetworkProtocolNumber) AddressEndpoint {
	if len(localAddr) == 0 {
...
func (s *Stack) FindRoute(id tcpip.NICID, localAddr, remoteAddr tcpip.Address, netProto tcpip.NetworkProtocolNumber, multicastLoop bool) (Route, *tcpip.Error) {
	...
	if id != 0 && !needRoute {
		if nic, ok := s.nics[id]; ok && nic.Enabled() {
			if ref := s.getAddressEP(nic, localAddr, remoteAddr, netProto); ref != nil {
				return makeRoute(netProto, ref.AddressWithPrefix().Address, remoteAddr, nic, ref, s.handleLocal && !nic.IsLoopback(), multicastLoop && !nic.IsLoopback()), nil
			}
		}
	} else {
			...
			if nic, ok := s.nics[route.NIC]; ok && nic.Enabled() {
				if ref := s.getAddressEP(nic, localAddr, remoteAddr, netProto); ref != nil {
					if len(remoteAddr) == 0 {
						...
						remoteAddr = ref.AddressWithPrefix().Address
					}

					r := makeRoute(netProto, ref.AddressWithPrefix().Address, remoteAddr, nic, ref, s.handleLocal && !nic.IsLoopback(), multicastLoop && !nic.IsLoopback())
					if len(route.Gateway) > 0 {
						if needRoute {
							r.NextHop = route.Gateway
						}
					} else if subnet := ref.AddressWithPrefix().Subnet(); subnet.IsBroadcast(remoteAddr) {
...
func (s *Stack) CheckLocalAddress(nicID tcpip.NICID, protocol tcpip.NetworkProtocolNumber, addr tcpip.Address) tcpip.NICID {
	...
	for _, nic := range s.nics {
		if ref := nic.findEndpoint(protocol, addr, CanBePrimaryEndpoint); ref != nil {
			ref.DecRef()
			return nic.id
		}
	}
...
func (s *Stack) GetNetworkEndpoint(nicID tcpip.NICID, proto tcpip.NetworkProtocolNumber) (NetworkEndpoint, *tcpip.Error) {
	...
	return nic.networkEndpoints[header.IPv6ProtocolNumber], nil
}
...
func (s *Stack) FindNetworkEndpoint(netProto tcpip.NetworkProtocolNumber, address tcpip.Address) (NetworkEndpoint, *tcpip.Error) {
	...
	for _, nic := range s.nics {
		ref := nic.getAddressOrCreateTemp(netProto, address, NeverPrimaryEndpoint, none)
		if ref == nil {
			continue
		}

		ep := ref.NetworkEndpoint()
		ref.DecRef()
		return ep, nil
	}
